...
class KlikXXi : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val tags = doc.select(".gmr-movie-on a").asIterable().map { it.text().trim() }
        val actors = doc.select("span[itemprop=director] span[itemprop=name]").asIterable().map { it.text() }
        ...
        if (isSeries) {
            val episodes = doc.select("div.gmr-season-block").asIterable().flatMap { season ->
    ...
    override suspend fun loadLinks(
        ...
        if (!postId.isNullOrEmpty()) {
            document.select("div.tab-content-ajax").asIterable().forEach { tab ->
                ...
                response.select("iframe").asIterable().forEach { frame ->
