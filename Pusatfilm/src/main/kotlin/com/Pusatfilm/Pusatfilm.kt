...
class Pusatfilm : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val actors = document.select("div.gmr-moviedata").last()?.select("span[itemprop=actors]")?.asIterable()?.map { it.select("a").text() }
        val recommendations = document.select("ul > li").asIterable().mapNotNull { it.toRecommendResult() }
