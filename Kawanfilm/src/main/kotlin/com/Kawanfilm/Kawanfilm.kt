...
class Kawanfilm : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = document.select("article.item").asIterable().mapNotNull { it.toSearchResult() }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val actors = document.select("div.gmr-moviedata").asIterable().last()?.select("span[itemprop=actors]")?.asIterable()?.map { it.select("a").text() }
        ...
        return if (tvType == TvType.TvSeries) {
            val episodes = document.select("div.vid-episodes a, div.gmr-listseries a").asIterable().map { eps ->
    ...
    override suspend fun loadLinks(
        ...
        // 3. Tombol Download
        document.select("ul.gmr-download-list li a").asIterable().forEach { linkEl ->
