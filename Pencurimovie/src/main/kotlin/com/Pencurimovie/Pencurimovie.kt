...
class Pencurimovie : MainAPI() {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return document.select("div.ml-item").asIterable().mapNotNull { it.toSearchResult() }
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val actors = document.select("div.mvic-info p:contains(Actors)").select("a").asIterable().map { it.text() }
        val year = document.select("div.mvic-info p:contains(Release)").select("a").text().toIntOrNull()
        val recommendation = document.select("div.mlw-related div.ml-item").asIterable().mapNotNull { it.toSearchResult() }
