...
class Pahe : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val home = doc.select("article, div.post-item").asIterable().mapNotNull { article ->
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return doc.select("article, div.post-item").asIterable().mapNotNull { article ->
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val tags = doc.select("a[rel=category tag], a[href*=category]").asIterable().map { it.text() }.filter { it.isNotBlank() }
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        doc.select("a[href]").asIterable().filter { el ->
        ...
        doc.select("iframe").asIterable().mapNotNull { it.attr("src").ifBlank { null } }.forEach { src ->
